  ...
  <mj-body>
    ...
    <mj-section background-color="#fafafa">
      <mj-column width="400px">
        <mj-text font-size="20px" font-family="Roboto" color="#626262" align="center">${trans(ctx, 'mail.changeEmail.title')}</mj-text>
        <mj-text color="#525252" font-family="Roboto" align="center">${trans(ctx, 'mail.changeEmail.body')}</mj-text>
        <mj-button background-color="#F45E43" href="${config.hostname}user/change-email/${data.token}">${trans(ctx, 'mail.changeEmail.cta')}</mj-button>
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>
