...
/**
  * @remarks - [PUT] '/user/reset-password'
  *
  ...
  * 5. Error on new password no match confirm - 400 & error msg
  * 6. Success on valid token & update password - 200 & success msg & doc user
  * 7. Error on user not found - 401 & error msg
*/

testToken('put', '/user/reset-password')
...
describe('Private Routes _user', () => {
  ...
  describe('[PUT] /user/reset-password', () => {
    ...
    */
    test('should return 400 & \'new password in incorrect format\'', async () => {
      const token = JWTOKEN.sign({ id: user._id }, config.jwtSecret, { expiresIn: 86400 })
      const res = await REQUEST(server)
        .put('/user/reset-password')
        .set('Content-Type', 'application/json')
      ...
      expect(res.status).toEqual(400)
      expect(res.type).toEqual('application/json')
      expect(res.body.msg).toEqual('new password in incorrect format')
    })

    /**
     * 4. Error on field no match regex
    */
    test('should return 400 & \'new password should contain at least one digit...\'', async () => {
      const token = JWTOKEN.sign({ id: user._id }, config.jwtSecret, { expiresIn: 86400 })
      const res = await REQUEST(server)
        .put('/user/reset-password')
        .set('Content-Type', 'application/json')
      ...
      expect(res.status).toEqual(400)
      expect(res.type).toEqual('application/json')
      expect(res.body.msg).toEqual('new password should contain at least one digit, one lower case, one upper case and at least 8 characters')
    })

    /**
     * 5. Error on new password no match confirm
    */
    ...
    /**
     * 6. Success on valid token & update password
    */
